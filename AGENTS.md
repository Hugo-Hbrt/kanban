...
- Packaged or GUI launches can still have a reduced PATH even when the user's shell setup is correct. Keep agent detection and launch shell-free, but augment PATH with a small set of user-local executable directories like `~/.local/bin` and `~/bin` so install checks and runtime launch agree without broadening detection to unrelated system installs.
