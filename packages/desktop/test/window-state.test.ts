...
describe("migrateWindowStateIfNeeded", () => {
	...
	it("removes the legacy file after a successful migration", () => {
		// The migration is gated on `multi` not existing, so leaving the
		// legacy around would be functionally harmless. We delete it anyway
		// to avoid leaving dead state on disk; this test guards that
		// cleanup so the migration stays additive-plus-cleanup rather than
		// silently regressing to additive-only.
		const legacyPath = resolveWindowStatePath(tmpDir);
		writeFileSync(legacyPath, JSON.stringify(SAMPLE_LEGACY_STATE), "utf-8");
