...
/**
 * Windows-specific PATH dirs that depend on env vars resolved at call time.
 * Computed lazily (unlike the macOS/Linux static list) because APPDATA /
 * LOCALAPPDATA / ProgramFiles paths vary per user.
 */
function getWindowsExtraPathDirs() {
    const dirs = [];
    ...
    if (appData)
        dirs.push(join(appData, "npm")); // npm global
    if (localAppData) {
    ...
    if (programFiles)
        dirs.push(join(programFiles, "Git", "cmd"));
    if (programFilesX86)
        dirs.push(join(programFilesX86, "Git", "cmd"));
    return dirs;
}
/**
 ...
 */
const EXTRA_PATH_DIRS = process.platform === "darwin"
    ? [
        "/opt/homebrew/bin",
        "/opt/homebrew/sbin",
        "/usr/local/bin",
...
//# sourceMappingURL=runtime-child-env.js.map